...
{
    public static List<Transform> _linqCache = new List<Transform>();
...
//                Debug.Log(i);
    ...
    {
        _linqCache.Clear();
        if (parent == null) return _linqCache;

        foreach (Transform child in parent.transform)
        {
            _linqCache.Add(child);
        }

        return _linqCache.OrderBy(x => Random.value).Take(count).ToList();
    }
